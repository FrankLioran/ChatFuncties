...
# -----------------------------
# 2. Zoeklogica (Robuust & Slim)
# -----------------------------

def search_wikipedia(q: str):
    """Zoekt eerst naar de best passende paginatitel en haalt daarna de samenvatting op."""
    try:
        headers = {"User-Agent": "EvaAssistant/1.0"}
        # Stap 1: Zoek op titel via OpenSearch API
        ...
        # Stap 2: Haal samenvatting op van de exact gevonden titel
...
def search_duckduckgo(q: str):
    """Haalt actuele zoeksnippets op via DuckDuckGo HTML Lite."""
    try:
...
def generate_ai_summary(q: str, context_text: str):
    """Gebruikt jouw ai_router om de zoekresultaten intelligent samen te vatten."""
    messages = [
        {
            "role": "system",
            "content": "Je bent Eva, de intelligente, warme en meedenkende AI-assistent van Frank. Vat de onderstaande zoekresultaten helder, overzichtelijk en bondig samen in het Nederlands."
        },
        {
            "role": "user",
